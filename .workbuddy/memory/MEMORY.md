...
- **GitHub 仓库**: lgm3299/github-wzgl

## 新增/修改文件（2026-06-21）
- `_redirects`：Cloudflare SPA 路由重定向（新增）
- `_headers`：CDN 缓存策略（新增）
- `.env.example`：环境变量示例（新增）
- `.gitignore`：防止敏感文件提交，新增 `.workbuddy/` 忽略（更新）
- `CLOUDFLARE_DEPLOY_GUIDE.md`：完整部署指南（已更新，420行）
- `README.md`：全面重写，反映当前架构、功能、部署方式（重写）
- **删除文件**：`vercel.json`、`.vercelignore`、`.vercel/` 目录、`SETUP_GUIDE.md`、`.github/workflows/deploy.yml`（空的 `.github` 目录也删除）、`dist/` 目录（与 Cloudflare Pages 模式无关）
